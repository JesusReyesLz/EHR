...
import { ModuleType, Patient, ClinicalNote, ConsultationRecord, PatientStatus } from './types';
...
const App: React.FC = () => {
  ...
  const addNote = (newNote: ClinicalNote) => {
    // Si la nota ya existe, la actualizamos; si no, la añadimos
    setNotes(prev => {
    ...
    if (patient) {
      const record: ConsultationRecord = {
        ...
        sex: patient.sex,
        age: patient.age,
        diagnosis: newNote.content.diagnosis || newNote.content.finalDiagnosis || 'Sin Dx específico',
        treatment: newNote.content.plan || newNote.content.dischargePlan || 'Revisar evolución en expediente',
  ...
  return (
    <Router>
      <Layout currentModule={currentModule} onModuleChange={setCurrentModule}>
        <Routes>
          <Route path="/" element={<Dashboard module={currentModule} patients={patients} onUpdateStatus={updatePatientStatus} onModuleChange={setCurrentModule} />} />
          <Route path="/new-patient" element={<NewPatient onAdd={addPatient} patients={[]} />} />
          <Route path="/edit-patient/:id" element={<NewPatient onAdd={updatePatient} patients={patients} />} />
          <Route path="/patient/:id" element={<PatientProfile patients={patients} notes={notes} onUpdatePatient={updatePatient} />} />
          
          {/* Rutas de Notas Médicas Especializadas (Creación y Edición) */}
          <Route path="/patient/:id/note/evolution" element={<EvolutionNote patients={patients} onSaveNote={addNote} />} />
          <Route path="/patient/:id/note/evolution/:noteId" element={<EvolutionNote patients={patients} onSaveNote={addNote} />} />
          
          <Route path="/patient/:id/note/emergency" element={<EmergencyNote patients={patients} onSaveNote={addNote} />} />
          <Route path="/patient/:id/note/emergency/:noteId" element={<EmergencyNote patients={patients} onSaveNote={addNote} />} />
          
          <Route path="/patient/:id/note/surgical" element={<SurgicalNote patients={patients} onSaveNote={addNote} />} />
          <Route path="/patient/:id/note/surgical/:noteId" element={<SurgicalNote patients={patients} onSaveNote={addNote} />} />
          
          <Route path="/patient/:id/note/esavi" element={<ESAVINote patients={patients} onSaveNote={addNote} />} />
          <Route path="/patient/:id/note/esavi/:noteId" element={<ESAVINote patients={patients} onSaveNote={addNote} />} />
          
          <Route path="/patient/:id/note/discharge" element={<DischargeNote patients={patients} onSaveNote={addNote} />} />
          <Route path="/patient/:id/note/discharge/:noteId" element={<DischargeNote patients={patients} onSaveNote={addNote} />} />
          
          <Route path="/patient/:id/note/interconsulta" element={<InterconsultaNote patients={patients} onSaveNote={addNote} />} />
          <Route path="/patient/:id/note/interconsulta/:noteId" element={<InterconsultaNote patients={patients} onSaveNote={addNote} />} />

          <Route path="/patient/:id/history" element={<MedicalHistory patients={patients} onUpdatePatient={updatePatient} onSaveNote={addNote} />} />
          <Route path="/patient/:id/nursing-sheet" element={<NursingSheet patients={patients} onSaveNote={addNote} />} />
          <Route path="/patient/:id/auxiliary-report" element={<AuxiliaryReport patients={patients} onSaveNote={addNote} />} />
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900">
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-20 lg:w-72 bg-white border-r border-slate-200 flex flex-col fixed left-0 top-16 h-[calc(100vh-64px)] z-50">
