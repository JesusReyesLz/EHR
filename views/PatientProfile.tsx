...
const PatientProfile: React.FC<{ patients: Patient[], notes: ClinicalNote[], onUpdatePatient: (p: Patient) => void, onSaveNote: (n: ClinicalNote) => void, doctorInfo: DoctorInfo }> = ({ patients, notes, onUpdatePatient, onSaveNote, doctorInfo }) => {
  ...
  const historyData = useMemo(() => {
    ...
    const pattern = `${dayInt}/${monthInt}/${y}`; 
  ...
  const handleSaveMonitoringSheet = () => {
    ...
    if (!hasVitals && !hasDiuresis) {
      alert("No se detectaron registros para la fecha seleccionada. Realice una toma antes de sellar.");
      return;
    }
    ...
    const newNote: ClinicalNote = {
      ...
      type: 'Hoja de Monitorización y Sábana de Signos',
      ...
      content: {
        ...
        summary: `Reporte de vigilancia continua. Registros procesados: ${historyData?.length || 0} tomas de signos, ${dailyDiuresis.length} registros de eliminación.`
      },
      isSigned: true,
    ...
    // Feedback visual y apertura automática de previsualización
    setTimeout(() => {
      setSelectedNote(newNote);
      const toast = document.createElement('div');
      toast.className = 'fixed bottom-10 left-1/2 -translate-x-1/2 bg-emerald-600 text-white px-10 py-5 rounded-3xl font-black text-xs uppercase tracking-widest shadow-2xl z-[300] animate-in fade-in slide-in-from-bottom-4';
      toast.innerHTML = 'Documento de Monitoreo Sellado y Archivando';
           ...
           <div className="bg-white border border-slate-200 rounded-[3rem] p-8 shadow-sm relative overflow-hidden">
              <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                 <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-[0.3em] flex items-center gap-3">
                    <TrendingUp className="text-blue-600" /> Gráfica de Monitoreo Dinámico
                 </h3>
              ...
              <div className="p-8 bg-slate-50 border-b border-slate-200 flex justify-between items-center no-print">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       ...
                       <p className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">Certificación Digital NOM-024</p>
                    </div>
                 </div>
