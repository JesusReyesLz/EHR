...
import { 
  ...
  MoveHorizontal, Truck, FileText,
  Baby, Heart, HeartPulse, Thermometer, 
  LayoutGrid, ClipboardList, LogOut, AlertCircle
} from 'lucide-react';
...
const HospitalMonitor: React.FC<HospitalMonitorProps> = ({ patients, onUpdatePatient }) => {
  ...
  const handleCompleteMove = (newBedId: string, targetModule: ModuleType) => {
    ...
    onUpdatePatient({
      ...
      status: targetModule === ModuleType.HOSPITALIZATION ? PatientStatus.ADMITTED : PatientStatus.IN_CONSULTATION
    });
  ...
  const renderSidebarPending = (moduleFilter: ModuleType, themeColor: string) => {
    // Un paciente está pendiente si pertenece al módulo activo Y no tiene ubicación (bedNumber) asignada
    const pending = patients.filter(p => p.assignedModule === moduleFilter && !p.bedNumber && p.status !== PatientStatus.ATTENDED);
                    ...
                    >
                       ...
                       {activePatient ? (
                          ...
                          <div className="flex flex-col items-center justify-center flex-1 gap-2 text-slate-300">
                             ...
                             <p className="text-[8px] font-black uppercase tracking-widest">Asignar Aquí</p>
                          </div>
                       )}
                    </div>
                 );
        ...
        <div className="bg-slate-900 rounded-[3.5rem] p-10 shadow-2xl border-4 border-slate-800">
            <div className="flex justify-between items-center mb-10">
               <h3 className="text-xl font-black text-white uppercase tracking-tight flex items-center">
                  <Flame className="w-6 h-6 mr-3 text-rose-500" />Boxes de Urgencias
               </h3>
               {patientInTransit && (
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
        <div className="space-y-2">
          ...
          <h1 className="text-5xl font-black text-slate-900 tracking-tighter uppercase leading-none">
            Monitor <span className="text-blue-600 italic">Hospitalario</span>
          </h1>
        </div>
      ...
      {patientInTransit && (
        <div className="bg-slate-900 text-white p-6 rounded-[2rem] shadow-2xl border-b-4 border-amber-500 animate-in slide-in-from-top-4 flex flex-col md:flex-row items-center justify-between gap-6">
           <div className="flex items-center gap-6">
              ...
              <div>
                 <p className="text-[10px] font-black text-amber-500 uppercase tracking-[0.2em]">ASIGNACIÓN DE ESPACIO ACTIVA</p>
                 <h4 className="text-lg font-black uppercase tracking-tight">{patientInTransit.name}</h4>
              </div>
           </div>
           <button 
             onClick={() => { setPatientInTransit(null); window.history.replaceState({}, document.title); }}
